...
class JSONHandler(FileHandler):
    ...
    def write(self, data):
        ...
        if self.logger:
            self.logger.append(f"Writing JSON file at path: {self.output_file}")
    ...
    def append(self, data):
        if self.logger:
            self.logger.append(f"Appending JSON file at path: {self.output_file}")
    ...
    def format(self):
        "Format JSON file"
        if self.logger:
            self.logger.append(f"Formatting JSON file at path: {self.output_file}")
        data = self.read()
